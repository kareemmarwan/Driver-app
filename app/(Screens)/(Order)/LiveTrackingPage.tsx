...
// خط سير حالة المندوب والطلب مترجم للعربية ومعد ليدعم الـ RTL
...
export default function LiveTrackingPage() {
  ...
  // حساب نسبة تقدم الشريط ديناميكياً بناءً على الخطوة النشطة الحالية
  ...
  // تأثير المحاكاة الحية لانجراف وحركة السائق (Live Drift Effect) عبر الشاشة
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden font-sans select-none bg-slate-50 text-slate-900" dir="rtl">
      ...
      {/* شريط التطبيق العلوي الزجاجي النظيف */}
      ...
      <main className="relative z-0 w-full h-full bg-slate-100">
        ...
        {/* عناصر الـ Markers والمسارات فوق الخريطة */}
        <div className="absolute inset-0 pointer-events-none">

          {/* ١. نقطة انطلاق الاستلام (المتجر) */}
          <div className="absolute z-10 flex flex-col items-center translate-x-1/2 -translate-y-1/2 top-1/4 right-1/4">
            <div className="flex items-center justify-center w-8 h-8 border-2 border-white rounded-full shadow-lg bg-emerald-600">
          ...
          {/* ٢. أيقونة المندوب الحية العائمة ذات النبض الحركي */}
          <div
            ref={driverRef}
          ...
          >
            <div className="relative flex items-center justify-center">
              <div className="absolute w-16 h-16 duration-1000 rounded-full bg-emerald-500/20 animate-ping" />
              <div className="absolute w-20 h-20 duration-700 rounded-full bg-emerald-500/10 animate-pulse" />
              <div className="flex items-center justify-center text-white border-4 border-white rounded-full shadow-2xl w-14 h-14 bg-emerald-600">
      ...
      <div className={`fixed bottom-0 left-0 right-0 max-w-md mx-auto z-40 bg-white rounded-t-[32px] shadow-[0px_-10px_40px_rgba(15,23,42,0.08)] border-t border-slate-100 transition-all duration-300 ${isSheetExpanded ? 'h-[75vh]' : 'h-auto'}`}>
        ...
        {/* مقبض اللوحة العلوي الجمالي - قابل للضغط للتوسيع */}
        <div
        ...
        <div className="px-5 pb-8 space-y-6 overflow-y-auto max-h-[calc(100%-24px)]">
          ...
          <div className="flex items-end justify-between">
            ...
            <div className="flex items-center gap-1 px-3 py-2 text-xs font-bold border rounded-full shadow-sm text-emerald-700 bg-emerald-50 border-emerald-100 animate-pulse">
              <span className="text-base font-bold material-symbols-outlined">bolt</span>
              <span>توصيل إكسبرس</span>
            </div>
          </div>

          {/* شريط خط التايم لاين الأفقي لتطور حالة التوصيل (RTL المطور) */}
          <div className="relative flex items-start justify-between pt-2">
            {/* خط التوصيل الخلفي الرمادي الأساسي */}
            <div className="absolute top-[18px] right-4 left-4 h-[3px] bg-slate-100 rounded-full z-0" />

            {/* خط التوصيل الفعلي الأخضر المتقدم ديناميكياً */}
            <div
              className="absolute top-[18px] right-4 h-[3px] bg-emerald-600 rounded-full z-0 transition-all duration-500"
              style={{ width: `calc(${progressPercentage}% - 16px)` }}
            />

            {/* تفاصيل وعُقد المراحل الفردية */}
              ...
              <div key={step.id} className={`flex flex-col items-center relative z-10 w-1/5 ${!step.active && 'opacity-35'}`}>
                {step.current ? (
                  <div className="relative flex items-center justify-center w-9 h-9">
                    ...
                    <div className="flex items-center justify-center w-8 h-8 border-2 border-white rounded-full shadow-md bg-emerald-600">
                ...
                <span className={`text-[10px] font-bold text-center mt-2 tracking-tight ${step.current ? 'text-emerald-600 font-extrabold' : 'text-slate-600'}`}>
                  {step.label}
                </span>
              </div>
            ))}
          ...
          {/* بطاقة معلومات السائق بتصميم عصري مسطح */}
          <div className="flex items-center justify-between p-4 border shadow-sm bg-slate-50 rounded-2xl border-slate-100">
            ...
            <div className="flex gap-2.5 shrink-0">
              ...
              <button className="flex items-center justify-center text-white transition-all rounded-full shadow-md w-11 h-11 bg-emerald-600 shadow-emerald-600/10 hover:bg-emerald-700 active:scale-90">
                <span className="text-lg material-symbols-outlined" style={{ fontVariationSettings: "'FILL' 1" }}>call</span>
              </button>
            </div>
          </div>

          {/* ملخص المشتريات السريع */}
          <div className="flex items-center justify-between pt-4 text-xs border-t border-slate-100">
            ...
            <button className="px-2 py-1 font-bold transition-colors rounded-lg text-emerald-600 hover:text-emerald-700 hover:bg-emerald-50">عرض الفاتورة</button>
          </div>

        </div>
      </div>

      {/* إضافة أنيميشن مخصص لحركة النقاط بالـ CSS في حال لم تكن معرفة */}
      <style jsx global>{`
        @keyframes dash {
          to {
            stroke-dashoffset: 0;
